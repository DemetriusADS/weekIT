    ...
    */
    ...
    public function carregarMonitorias(){
        $monitorias = DB::table('atividade_has_monitor')
            ->select('atividade_has_monitor.monitor_id','atividade_has_monitor.atividade_id','participante.nome',
                'atividade.identificador', 'atividade.titulo',DB::raw('DATE_FORMAT(atividade.data_inicio,"%d/%m/%Y") as data'),
        ...
        foreach ($datas_monitorias as $data_monitoria) {
            foreach ($data_pretendida as $data_inscrever){
                if (date_create($data_monitoria->data_inicio) == date_create($data_inscrever->data_inicio)){
                    if ($this::intervaloEntreDatas($data_monitoria->hora_inicio, $data_monitoria->hora_fim, 
